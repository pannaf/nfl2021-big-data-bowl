...
from motion_models.dataloader_team_analysis_PLAYER import PlayDataset
import numpy as np
...
def evaluate(dataloader, model, event_type):
    with torch.no_grad():
        ...
        for (local_batch,local_labels,local_games,local_plays, local_ids) in dataloader:
            # Transfer to GPU
            ...
            nflIds.extend(local_ids.cpu().numpy().tolist())
...
im_rootdir = '/data2/Code/nfl_analytics/2021/trajpict_PLAYER/pictTraj_centered_from_snap/{}'.format(event_type)
...
ckpt_path = '/data2/Code/nfl_analytics/2021/nfl2021/pass_outcome_models/{}.ckpt'
...
results_df.to_csv('{}_pass_result_prob_PLAYER_full.csv'.format(event_type), index=False)
